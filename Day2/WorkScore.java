...
class WorkScore {
    ...
    public static int calc(int T,int E,int l,int u,int w_g){
        if(T<l && E>=w_g){
            return -2;
        }
        if(T<l && E<w_g){
    ...
    public static void main(String[] args) {
        ...
        score += calc(delt, dele, lower, upper , work_goal);
        
        for(int i = k ; i < n ; i++){
            delt += tasks[i] - tasks[i-k];
            dele += hours[i] - hours[i-k];
